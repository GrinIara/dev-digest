 ...
 *   `openrouter · deepseek/deepseek-v4-flash — $0.14/$0.28 per 1M · 1M ctx`
 *
 * Used by the agent editor (ConfigTab, cross-provider picker) and the Settings →
 * Feature Models picker so both render provider + OpenRouter prices identically.
 * The provider prefix is always shown: once a picker spans more than one
 * provider (ConfigTab's `useAllModels()`), model ids alone aren't unique.
 */
...
/** Build the model dropdown label: provider prefix, then price + context window when available. */
export function modelLabel(m: PricedModel): string {
  ...
  const prefix = `${m.provider} · ${m.id}`;
  return parts.length ? `${prefix} — ${parts.join(" · ")}` : prefix;
}

/** Build SearchableSelect/SelectInput options: every model gets a provider-prefixed label. */
